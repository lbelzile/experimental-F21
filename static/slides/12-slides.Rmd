...
Total effect is 
$$\mathsf{TE}(x, x^*) = \textsf{NDE}(x, x^*) - \textsf{NIE}(x^*, x)$$
???
...
Compute the sample proportion of bootstrap statistics $S^{(1)}, \ldots, S^{(B)}$ that are more 'extreme' than $S$ 

$$\frac{1}{B}\sum_{b=1}^B \mathsf{1}(|S^{(b)}| > |S|)$$
