...
- $\mathrm{G}^{\star}\mathrm{Power}$ and **R** packages takee Cohen's $f$ (or $f^2$) as input .
...
In **R**, if `a` and `b` denote the number of levels of each factor and `omega.sq` is $\omega^2_{\langle AB \rangle}$, then the sample size needed to detect this effect with power of 80% is
